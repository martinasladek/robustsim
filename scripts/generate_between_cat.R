...
names(between_cat_list) <- c("2x2 (with interaction)", "2+2 (no interaction)", 
                             "2x3 (with interaction)", "2+3 (no interaction)",
                             "1-way (2 groups)", "3x3 (with interaction)", 
                             "3+3 (no interaction)", "1-way (3 groups)"
                             )
...
for(i in names(between_cat_list)){
  ...
  qmd_lines_i <- paste0(
    ...
    codechunk(
      code_string = 'source(here::here("scripts/generate_between_cat.R"))'
    ),  "\n\n",
  ...
  writeLines(qmd_lines_i, here::here(paste0("summary_qmds/between_cat_", i ,".qmd")))
